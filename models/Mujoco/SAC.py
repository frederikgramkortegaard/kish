...
from models.Atari.Optimizer import RNAdamWP, SGDNorm
...
class Agent():
    ...
    def update_critic_(self, critic, optim, state, action, reward, next_state, mask):
        ...
        ortholoss += (torch.sqrt((self.critic1.fc1.weight.t() ** 2).sum(-1, keepdim=True)) - 1).pow(2).mean()

        loss = torch.mean(0.5 * (Q_values - Q_target).pow(2)) + 0.01 * ortholoss # Equation 5 from SAC
    ...
    def update(self):
        ...
        ortholoss += (torch.sqrt((self.actor.fc1.weight.t() ** 2).sum(-1, keepdim=True)) - 1).pow(2).mean()
        loss = (self.temp.detach() * a_log_prob - q).mean() + 0.01 * ortholoss # Update with equation 9 from SAC paper
        ...
        while t <= timesteps:
            ...
            while not done and not truncated:
                ...
                if (t % 100) == 0:
                    ...
                    print('actor weight norm: ', torch.sqrt((self.actor.fc1.weight.t() ** 2).sum(-1, keepdim=True)).mean().item())
