...
class network(nn.Module):
    def __init__(self, heads, hidden_layers, inputs, outputs):
        ...
        self.decoder = AttentionSplit(inputs=hidden_layers, hidden_size=hidden_layers, n_heads=8)
...
class network3(nn.Module):
    def __init__(self, hidden_layers, inputs, outputs):
        ...
        self.encoder = nn.TransformerEncoderLayer(hidden_layers, 4, hidden_layers, activation='gelu', batch_first=True)
        self.decoder = nn.TransformerEncoderLayer(hidden_layers, 8, hidden_layers, activation='gelu', batch_first=True)
        self.classi = nn.Linear(hidden_layers, outputs)
...
class agent():
    def __init__(self, data):
        ...
        self.optim3 = torch.optim.AdamW(self.network3.parameters(), lr=0.0003, weight_decay=1e-4, amsgrad=True, fused=True)
