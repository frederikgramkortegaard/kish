...
class AttentionSplit(nn.Module):
    def __init__(self, inputs, hidden_size, n_heads):
        ...
        self.cells = nn.Parameter(
            torch.ones((self.n_heads, int(self.hidden_size / self.n_heads)))
        )
        nn.init.normal_(self.cells, 0, math.sqrt(2 / nc))

        na = self.n_heads * int(int(self.hidden_size * self.n_heads) / self.inputs)
        self.cell_attention = nn.Parameter(
            torch.ones(
                (
                    self.n_heads,
                    int(int(self.hidden_size * self.n_heads) / self.hidden_size),
                )
            )
        )
        ...
        self.head_weight = nn.Parameter(
            torch.ones(
                (self.n_heads, int(int(self.hidden_size * self.n_heads) / self.inputs))
            )
        )
        ...
        if (
            (self.inputs % self.n_heads) != 0
            or (self.hidden_size % self.n_heads) != 0
            or (self.inputs % self.n_heads) == 1
            or ((self.hidden_size * self.n_heads) / self.inputs) % 1 != 0
        ):
    ...
    def forward(self, inputs, hidden=None):
        ...
        current_input = inputs.view(
            inputs.shape[0], inputs.shape[1], self.n_heads, -1
        ).view(-1, self.n_heads)

        # Calculate the outputs for all timesteps in a matrix multiplication
        pure_outputs = self.f(
            self.first_norm(
                (current_input @ self.head_weight).view(bs, ls, self.n_heads, -1)
            )
        )

        if hidden is not None:
            hidden = self.f(hidden.view(-1, self.n_heads) @ self.cell_attention)
            hidden = self.cell_attention_norm(
                hidden.view(bs, ls, self.n_heads, int(self.hidden_size / self.n_heads))
            )

        temporals = torch.zeros(
            bs, self.n_heads, int(self.hidden_size / self.n_heads)
        ).to(device)
        output_list = []
        context_window = []
        ...
        if hidden is None:
            for i in range(inputs.shape[1]):
                ...
                context_outputs = torch.einsum(
                    "nhi, bhj->bhj", self.cells.unsqueeze(0), self.dropout(temporals)
                )
            ...
            for i in range(inputs.shape[1]):
                ...
                context_outputs = torch.einsum(
                    "nhi, bhj->bhj",
                    self.cells.unsqueeze(0),
                    self.dropout(temporals + hidden[:, i, :, :]),
                )
                context_outputs = self.dropout(self.f(context_outputs))
