    ...
    def store_grad_fisher_avg(self):
        for group in self.param_groups:
            for p in group["params"]:
                ...
                avg_grad.mul_(1 - group["factor"]).add_(
                    grad.detach().clone(), alpha=group["factor"]
                )

                avg_weight = state["weight_avg"]
                avg_weight.mul_(1 - group["factor"]).add_(
                    p.data.detach().clone(), alpha=group["factor"]
                )

                avg_fisher = state["fisher_avg"]
                avg_fisher.mul_(1 - group["factor"]).add_(
                    avg_grad**2, alpha=group["factor"]
                )
    ...
    def compute_EWC_loss(self):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                ewc_sum.append(
                    ((avg_fisher * (p.data - avg_weight) ** 2)).sum() * group["lambd"]
                )
