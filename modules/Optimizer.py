        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                update = torch.linalg.solve(
                    H * torch.eye(J.shape[0], device=J.device), J.flatten(1)
                )
...
class SGD(torch.optim.Optimizer):
    def __init__(
        self, params, lr=0.1, momentum=0, dampening=0, weight_decay=1e-4, nesterov=False
    ):
        defaults = dict(
            lr=lr,
            momentum=momentum,
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if len(p.shape) > 1:
                    h_t = (
                        buf.unsqueeze(1).flatten(1) @ buf.unsqueeze(1).flatten(1).t()
                    ).diag().sqrt() * math.sqrt((4096 - 128) / (4096 * 128))
                    h_t *= torch.normal(
                        0, math.sqrt(2 / h_t.numel()), h_t.shape, device=p.device
                    )
                    h_t *= group["lr"]
                    h_t = h_t.unsqueeze(1).expand(
                        p.data.shape[0], p.data.unsqueeze(1).flatten(1).shape[1]
                    )
                    # p.data.add_(h_t)
                ...
                if len(p.shape) > 1:
                    ...
                    if torch.nn.functional.cosine_similarity(
                        p_flat, update_flat, dim=0
                    ).abs().max() < 0.1 / torch.numel(p.data):
                        update_flat = (
                            update_flat
                            - (
                                torch.linalg.vecdot(update_flat, p_flat)
                                / torch.linalg.vecdot(p_flat, p_flat)
                            )
                            * p_flat
                        )
...
class NSGD(torch.optim.Optimizer):
    def __init__(
        self, params, lr=0.1, momentum=0, dampening=0, weight_decay=1e-4, nesterov=True
    ):
        defaults = dict(
            lr=lr,
            momentum=momentum,
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if len(p.shape) > 1:
                    h_t = (
                        grad.unsqueeze(1).flatten(1) @ grad.unsqueeze(1).flatten(1).t()
                    ).diag().sqrt() * math.sqrt((2048 - 128) / (2048 * 128))
                    h_t *= torch.normal(
                        0, math.sqrt(2 / p.data.numel()), h_t.shape, device=p.device
                    )
                    h_t *= group["lr"]
                    h_t = h_t.unsqueeze(1).expand(
                        p.data.shape[0], p.data.unsqueeze(1).flatten(1).shape[1]
                ...
                if nesterov:
                    if dampening != 0:
                        update = (
                            (momentum * buf) / (1 - momentum ** (state["step"] + 1))
                        ) + (
                            ((1 - momentum) * grad) / (1 - momentum ** (state["step"]))
                        )
                    else:
                        update = grad + momentum * buf
    ...
    def step(self):
        for group in self.param_groups:
            for p in group["params"]:
                ...
                # momentum = ((0.9 * momentum) / (1 - 0.9 ** (state['step'] + 1))) + (((1 - 0.9) * J ) / (1 - 0.9 ** (state['step'])))
                if momentum2.max() / 0.1**2 >= 1 / torch.numel(
                    momentum
                ):  # We use the chernoff bound to give an upperbound of deviation from the mean
                    momentum = (momentum) / momentum2.sqrt().add(1e-8)

                momentum = ((0.9 * momentum) / (1 - 0.9 ** (state["step"] + 1))) + (
                    ((1 - 0.9) * J) / (1 - 0.9 ** (state["step"]))
                )
                update = momentum
    ...
    def step(self):
        for group in self.param_groups:
            for p in group["params"]:
                ...
                momentum, momentum2, momentum2_max = (
                    state["momentum"],
                    state["momentum2"],
                    state["momentum2_max"],
                )
                ...
                mu = beta1 * (
                    1.0 - 0.5 * (0.96 ** (state["step"] * group["momentum_decay"]))
                )
                mu_next = beta1 * (
                    1.0
                    - 0.5 * (0.96 ** ((state["step"] + 1) * group["momentum_decay"]))
                )
                ...
                grad = (J * (1.0 - mu)) / (1.0 - mu_product)
                momentum = (momentum * mu_next) / (1.0 - mu_product_next) + grad
                momentum = (momentum) / momentum2.sqrt().add(1e-8)
    ...
    def step(self):
        for group in self.param_groups:
            for p in group["params"]:
                ...
                ht = (
                    mt / mt.pow(2).sum(1).sqrt().unsqueeze(1).expand(-1, mt.shape[1])
                ) * ht.unsqueeze(1).expand(-1, mt.shape[1])
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                delta = torch.linalg.solve(
                    H * torch.eye(H.shape[0], device=p.device) * 0.1, mt.flatten(1)
                )
        ...
        for group in self.param_groups:
            ...
            for param in group["params"]:
                ...
                for j in range(len(self.prev_gradients) - 1, -1, -1):
                    ...
                    print("....")
                    alpha = torch.dot(
                        flat_param - param.flatten(0), flat_grad
                    ) / torch.dot(flat_grad, flat_grad)
                    q += alpha * flat_grad
                    z += alpha * (
                        flat_grad
                        - torch.dot(flat_grad, z)
                        / torch.dot(flat_grad, flat_grad)
                        * flat_grad
                    )

                gamma = torch.dot(
                    self.prev_parameters[-1] - param.view(-1), self.prev_gradients[-1]
                ) / torch.dot(self.prev_gradients[-1], self.prev_gradients[-1])
            ...
            for p in group["params"]:
                if p.grad is not None:
                    ...
                    if len(p.shape) > 1:
                        ...
                        if (
                            torch.nn.functional.cosine_similarity(last, first, dim=0)
                            .abs()
                            .max()
                            < 0.1
                        ):
                            first = (
                                first
                                - (
                                    torch.linalg.vecdot(first, last)
                                    / torch.linalg.vecdot(last, last)
                                )
                                * last
                            )
                            grad_0 = p.grad.data[0].flatten(0) / torch.linalg.norm(
                                p.grad.data
                            )
                    ...
                    if perturbed_params.grad is not None:
                        ...
                        H = (
                            (
                                perturbed_params.grad.data.t()
                                * perturbed_params.grad.data
                            )
                            .t()
                            .diag()
                        )
                ...
                for p in group["params"]:
                    ...
                    if (
                        ...
                        update_flat = (
                            ...
                            - (
                                torch.linalg.vecdot(update_flat, p_flat)
                                / torch.linalg.vecdot(p_flat, p_flat).add(1e-18)
                            )
                            * p_flat
                        )
                        update = update_flat.view(p.data.shape)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                buf.mul_(momentum).add_(grad, alpha=1 - dampening)
                delta = 0.9
                if buf.var(correction=0) / delta**2 >= 0.1 / math.sqrt(
                    torch.numel(buf)
                ):  # We use the chernoff bound to give an upperbound of deviation from the mean
                    buf.mul_(0.1)
                if nesterov:
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                perturb = (exp_avg / (1 - beta1 ** state["step"])) / (
                    max_exp_avg_sq / (1 - beta2 ** state["step"])
                ).sqrt().add(group["eps"])
                ...
                if len(p.shape) > 1:
                    perturb, wd_ratio = self._projection(
                        p, perturb, group["delta"], group["wd_ratio"], group["eps"]
                    )
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                delta = 0.2
                if (
                    buf.var(correction=0) / delta**2 >= 0.1
                ):  # We use the chernoff bound to give an upperbound of deviation from the mean
                    buf.mul_(0.1)
